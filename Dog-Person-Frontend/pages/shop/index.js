...
const shop = () => {
    ...
    useEffect(
        () => {
            if (router?.query?.type !== undefined && router?.query?.subtype !== undefined && router?.query?.page !== undefined) {
    ...
    const handleIncrement = () => {
        ...
        router.push(`/shop?type=${router?.query?.type}&subtype=${router?.query?.subtype}&page=${newPage}`)
    }
    ...
    const handlePageClick = (page) => {
        router.push(`/shop?type=${router?.query?.type}&subtype=${router?.query?.subtype}&page=${page}`)
    }
    return (
        <div>
            ...
            <div className={styles.shopContainer}>
                {
                    ...
                    // )
                    products?.map((product) =>
                        <ShopCard image={product.picture} name={product.name} price={product.price} avgrating={product.avg_rating} ratingcount={product.rating} paragraph={product.about} id={product._id} />
                    )
                }
            </div>
            {/* Page count. */}
